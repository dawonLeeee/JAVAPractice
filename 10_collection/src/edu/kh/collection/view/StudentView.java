...
public class StudentView {
	...
	public void addStudent() {
		...
		while(true) {
			...
			else System.out.println("[error] M / F 만 입력해주세요\n");
		}
		...
		if(service.addStudent(name, age, address, gender, score))
			...
			System.out.println("[error] 학생정보 추가 실패");
	...
	public void selectAll() {
		List<Student> stdList = service.getStdList(); // service에서 getter 생성
		
		// 향상된 for문
		// - 배열 또는 컬렉션의 모든 요소를
		// 	 처음부터 끝까지 순차접근할 때 사용하는 for문(forEach문)
		
		// for(요소 하나를 꺼내서 참조할 변수 : stdList)
	...
	public void updateStudent() {
		...
		if(service.updateStudent(index, address, score))
			System.out.println("[학생 정보 업데이트 성공]");
		else
			System.out.println("[error] 인덱스 불일치");
	...
	private void removeStudent() {
		...
		if(s == null) System.out.println("[error] 인덱스 불일치");
		else System.out.println(s.getName() + "학생 정보가 삭제되었습니다.");
