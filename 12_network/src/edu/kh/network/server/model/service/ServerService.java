...
public class ServerService {
	 ...
	 * 
	 * - 데이터 전송 순서가 보장되고, 수신 여부를 판단하여 오류(손실) 발생시 재전송함
	 * 
	 * java.net 패키지에서 제공하는 Socket, ServerSocket 클래스를 사용해야함
	 * 
	 * Socket
	 * - 프로세스의 양 끝단(프로세스간의 통신을 담당)
	 * - InputStream/OutputStream을 보유하고 있음
	 * 
	 * ServerSocket
	 * - 포트와 연결되어 외부 요청을 기다리는 객체
	 * - 클라이언트의 연결요청이 들어오면 클라이언트의 소켓을 얻어와 연결
	 * 
...
//		1.  서버의  포트번호  정함
		...
		BufferedReader br = null; // 속도향상을 위해 보조스트림 사용
...
//			4. 접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
			...
			// ->클라이언트의 요청이 오고 요청 수락이 될 때까지 무한대기
...
//			6.  보조  스트림을  통해  성능  개선
			...
			// 문자기반 BufferedReader와 바이트기반 InputStream를 연결할 수 있도록
